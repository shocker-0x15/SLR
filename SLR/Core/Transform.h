...
public:
    ...
    virtual void sample(float time, StaticTransform* tf) const = 0;
    virtual bool isChained() const = 0;
    virtual Transform* copy(ArenaAllocator &mem) const = 0;
    virtual BoundingBox3D motionBounds(const BoundingBox3D &bb) const = 0;
...
class ChainedTransform : public Transform {
    const Transform* m_parent;
    const Transform* m_transform;
public:
    ChainedTransform(const Transform* parent, const Transform* transform) : m_parent(parent), m_transform(transform) {
    ...
    void reduce(ArenaAllocator &mem, std::vector<const Transform*> &reducedTFs) const;
    const Transform* reduce(ArenaAllocator &mem) const;
    ...
    void sample(float time, StaticTransform* tf) const override {
        ...
        while (current) {
            ...
            StaticTransform nextTF;
            current->sample(time, &nextTF);
            *tf = nextTF * *tf;
