...
namespace SLR {
    ...
    float AshikhminSpecularBRDF::weightInternal(const SLR::BSDFQuery &query) const {
        ...
        return m_Rs.importance(query.wlHint) + (1 - m_Rs.importance(query.wlHint)) * std::pow(1.0f - dotHV, 5);
    }
    ...
    float AshikhminDiffuseBRDF::weightInternal(const SLR::BSDFQuery &query) const {
        return (2954.0f / 1863.0f * m_Rd.importance(query.wlHint) * (1 - m_Rs.importance(query.wlHint)) * (1.0f - std::pow(1.0f - std::fabs(query.dir_sn.z) / 2, 5)));
    }
