...
//        parser.traceParsing = true;
    ...
    Element Translate(const ParameterList &params, ErrorMessage* err) {
        static const Function proc{
            ...
            [](const std::map<std::string, Element> &args, ErrorMessage* err) {
                ...
                return Element(Type::Matrix, createShared<SLR::Matrix4x4>(SLR::translate(tx, ty, tz)));
            }
        };
    ...
    Element RotateX(const ParameterList &params, ErrorMessage* err) {
        static const Function proc{
            ...
            [](const std::map<std::string, Element> &args, ErrorMessage* err) {
                ...
                return Element(Type::Matrix, createShared<SLR::Matrix4x4>(SLR::rotateX(angle)));
            }
        };
        return proc(params, err);
    }
    ...
    Element RotateY(const ParameterList &params, ErrorMessage* err) {
        static const Function proc{
            ...
            [](const std::map<std::string, Element> &args, ErrorMessage* err) {
                float angle = args.at("angle").as<double>();
                return Element(Type::Matrix, createShared<SLR::Matrix4x4>(SLR::rotateY(angle)));
            }
        };
        return proc(params, err);
    }
    ...
    Element RotateZ(const ParameterList &params, ErrorMessage* err) {
        static const Function proc{
            ...
            [](const std::map<std::string, Element> &args, ErrorMessage* err) {
                float angle = args.at("angle").as<double>();
                return Element(Type::Matrix, createShared<SLR::Matrix4x4>(SLR::rotateZ(angle)));
            }
        };
        return proc(params, err);
    }
    ...
    Element Scale(const ParameterList &params, ErrorMessage* err) {
        static const Function proc0{
            ...
            [](const std::map<std::string, Element> &args, ErrorMessage* err) {
                ...
                return Element(Type::Matrix, createShared<SLR::Matrix4x4>(SLR::scale(s)));
            }
        };
        static const Function proc1{
            ...
            [](const std::map<std::string, Element> &args, ErrorMessage* err) {
                ...
                return Element(Type::Matrix, createShared<SLR::Matrix4x4>(SLR::scale(sx, sy, sz)));
            }
        };
    ...
    Element CreateSpectrum(const ParameterList &params, ErrorMessage* err) {
        static const Function proc0{
            ...
            [](const std::map<std::string, Element> &args, ErrorMessage* err) {
                ...
                return Element(Type::Spectrum, Spectrum::create(type, SLR::ColorSpace::sRGB, value, value, value));
            }
        };
        static const Function proc1{
            {
                {"type", Type::String, Element(Type::String, createShared<std::string>("Reflectance"))},
                {"space", Type::String, Element(Type::String, createShared<std::string>("sRGB"))},
                {"e0", Type::RealNumber},
                {"e1", Type::RealNumber},
                {"e2", Type::RealNumber}
            },
            [](const std::map<std::string, Element> &args, ErrorMessage* err) {
                ...
                return Element(Type::Spectrum, Spectrum::create(type, space, e0, e1, e2));
            }
        };
    ...
    Element CreateSpectrumTexture(const ParameterList &params, ErrorMessage* err) {
        static const Function proc0{
            ...
            [](const std::map<std::string, Element> &args, ErrorMessage* err) {
                ...
                return Element(Type::SpectrumTexture, createShared<ConstantSpectrumTexture>(spectrum));
            }
        };
    ...
    Element CreateMatte(const ParameterList &params, ErrorMessage* err) {
        static const Function proc{
            {{"reflectance", Type::SpectrumTexture}, {"sigma", Type::FloatTexture, Element(Type::FloatTexture, nullptr)}},
            [](const std::map<std::string, Element> &args, ErrorMessage* err) {
                ...
                return Element(Type::SurfaceMaterial, SurfaceMaterial::createMatte(reflectance, sigma));
            }
        };
        return proc(params, err);
    }
    ...
    Element CreateDiffuseEmitter(const ParameterList &params, ErrorMessage* err) {
        static const Function proc{
            ...
            [](const std::map<std::string, Element> &args, ErrorMessage* err) {
                ...
                return Element(Type::EmitterSurfaceProperty, SurfaceMaterial::createDiffuseEmitter(emittance));
            }
        };
        return proc(params, err);
    }
    ...
    Element CreateEmitterSurfaceMaterial(const ParameterList &params, ErrorMessage* err) {
        static const Function proc{
            ...
            [](const std::map<std::string, Element> &args, ErrorMessage* err) {
                ...
                return Element(Type::SurfaceMaterial, SurfaceMaterial::createEmitterSurfaceMaterial(mat, emit));
            }
        };
        return proc(params, err);
    }
    ...
    Element CreateMesh(const ParameterList &params, ErrorMessage* err) {
        static const Function proc{
            ...
            [](const std::map<std::string, Element> &args, ErrorMessage* err) {
                ...
                return Element(Type::Mesh, lightMesh);
            }
        };
        return proc(params, err);
    }
    ...
    Element CreateNode(const ParameterList &params, ErrorMessage* err) {
        static const Function proc{
            {},
            [](const std::map<std::string, Element> &args, ErrorMessage* err) {
                return Element(Type::Node, createShared<InternalNode>());
            }
        };
        return proc(params, err);
    }
