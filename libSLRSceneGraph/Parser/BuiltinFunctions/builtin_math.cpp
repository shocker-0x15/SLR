...
namespace SLRSceneGraph {
    namespace BuiltinFunctions {
        namespace Math {
            ...
            const Element max = Element::create<TypeMap::Function>(1,
                                                                   std::vector<ArgInfo>{{"x0", Type::RealNumber}, {"x1", Type::RealNumber}},
                                                                   [](const std::map<std::string, Element> &args, ExecuteContext &context, ErrorMessage* err) {
                                                                       float x0 = args.at("x0").raw<TypeMap::RealNumber>();
                                                                       float x1 = args.at("x1").raw<TypeMap::RealNumber>();
            ...
            const Element clamp = Element::create<TypeMap::Function>(1,
            ...
            const Element sqrt = Element::create<TypeMap::Function>(1,
                                                                    std::vector<ArgInfo>{{"x", Type::RealNumber}},
                                                                    [](const std::map<std::string, Element> &args, ExecuteContext &context, ErrorMessage* err) {
                                                                        float x = args.at("x").raw<TypeMap::RealNumber>();
                                                                        return Element(std::sqrt(x));
                                                                    });
            const Element pow = Element::create<TypeMap::Function>(1,
            ...
            const Element cos = Element::create<TypeMap::Function>(1,
                                                                   std::vector<ArgInfo>{{"x", Type::RealNumber}},
                                                                   [](const std::map<std::string, Element> &args, ExecuteContext &context, ErrorMessage* err) {
                                                                       float x = args.at("x").raw<TypeMap::RealNumber>();
                                                                       return Element(std::cos(x));
                                                                   });
            const Element tan = Element::create<TypeMap::Function>(1,
                                                                   std::vector<ArgInfo>{{"x", Type::RealNumber}},
                                                                   [](const std::map<std::string, Element> &args, ExecuteContext &context, ErrorMessage* err) {
                                                                       float x = args.at("x").raw<TypeMap::RealNumber>();
                                                                       return Element(std::tan(x));
                                                                   });
            const Element asin = Element::create<TypeMap::Function>(1,
                                                                    std::vector<ArgInfo>{{"x", Type::RealNumber}},
                                                                    [](const std::map<std::string, Element> &args, ExecuteContext &context, ErrorMessage* err) {
                                                                        float x = args.at("x").raw<TypeMap::RealNumber>();
                                                                        return Element(std::asin(x));
                                                                    });
            const Element acos = Element::create<TypeMap::Function>(1,
                                                                    std::vector<ArgInfo>{{"x", Type::RealNumber}},
                                                                    [](const std::map<std::string, Element> &args, ExecuteContext &context, ErrorMessage* err) {
                                                                        float x = args.at("x").raw<TypeMap::RealNumber>();
                                                                        return Element(std::acos(x));
                                                                    });
            const Element atan = Element::create<TypeMap::Function>(1,
            ...
            const Element dot = Element::create<TypeMap::Function>(1, 
            ...
            const Element cross = Element::create<TypeMap::Function>(1, 
                                                                     std::vector<ArgInfo>{{"v0", Type::Vector}, {"v1", Type::Vector}},
                                                                     [](const std::map<std::string, Element> &args, ExecuteContext &context, ErrorMessage* err) {
                                                                         const SLR::Vector3D &v0 = args.at("v0").raw<TypeMap::Vector>();
                                                                         const SLR::Vector3D &v1 = args.at("v1").raw<TypeMap::Vector>();
            ...
            const Element distance = Element::create<TypeMap::Function>(1, 
